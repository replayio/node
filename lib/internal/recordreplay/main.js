...
function Target_getCurrentMessageContents() {
  ...
  const error = process._recordReplayGetCurrentError();
...
function Target_topFrameLocation() {
  ...
  for (const location of frameLocations) {
    if (!process._recordReplayIgnoreScript(location.scriptId)) {
...
function getStackFrames() {
  ...
  for (const frame of callFrames) {
    if (!process._recordReplayIgnoreScript(frame.location.scriptId)) {
      frames.push(frame);
    }
  }
  return frames;
