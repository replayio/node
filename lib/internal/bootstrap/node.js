...
// Set up methods on the process object for all threads
{
  ...
  const recordreplay = {
    ...
    currentExecutionPoint: rawMethods.recordReplayCurrentExecutionPoint,
    currentExecutionPointURL() {
      const recordingId = rawMethods.recordReplayRecordingId();
      const point = rawMethods.recordReplayCurrentExecutionPoint();
      return `https://app.replay.io/recording/${recordingId}?point=${point}&hasFrames=true`;
    },
  ...
  ObjectDefineProperty(process, "recordreplay", {
    ...
    set(value) {
      ObjectDefineProperty(this, "recordreplay", {
        value,
        configurable: true,
        enumerable: true,
        writable: true
