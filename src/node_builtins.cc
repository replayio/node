...
MaybeLocal<Data> BuiltinLoader::LookupAndCompile(
  ...
  // Avoid using code cache when recording/replaying, as generated bytecode
  // depends on the running thread and we might end up running code compiled
  // by the main thread on a worker thread.  Leaving cached_data default means
  // has_cache stays false below, so the builtin is compiled fresh.
