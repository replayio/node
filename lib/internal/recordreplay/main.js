...
// Levels of console messages by the type of their Runtime.consoleAPICalled
// event. Other types are reported as "info".
...
function Pause_getAllFrames() {
  const frames = getStackFrames().map((frame, index) => {
    ...
    return createProtocolFrame(id, frame, index === 0);
  });
