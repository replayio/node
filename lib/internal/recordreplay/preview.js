...
ProtocolObjectPreview.prototype = {
  ...
  // Used to add properties related to the size of objects like typed arrays
  // and sets. The size isn't available from the object's own property
  // information, except by parsing the object description.
  addDescriptionProperty(name) {
    const match = /\((\d+)\)/.exec(this.obj.description || "");
    if (match) {
      const value = +match[1];
      this.addProperty({ name, value }, /* force */ true);
      return value;
    }
  },
};
...
function previewTypedArray() {
  this.addDescriptionProperty("length");
}

function previewSetMap(allProperties) {
  this.extra.containerEntryCount = this.addDescriptionProperty("size");
